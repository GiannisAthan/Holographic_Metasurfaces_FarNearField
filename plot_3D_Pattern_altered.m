function plot_3D_Pattern_altered( theta_deg, phi_deg, U_Lin, PlotType, IncDirec_deg )
...
if nargin ==5
    thetai_deg = IncDirec_deg(1);
    phii_deg = IncDirec_deg(2);
else
    thetai_deg = [];
    phii_deg = [];
end
...
for kp = 1:numel(PlotType)
    ...
    if PlotType(kp) <=3
        plot_3D_Spherical(theta_deg,phi_deg,r,PlotType(kp),thetai_deg,phii_deg) 
        title( [titstr, ' | Spher.'] );
    else
        plot_3D_Cylindrical(theta_deg,phi_deg,r,PlotType(kp),thetai_deg,phii_deg)
        title( [titstr, ' | Cyl.'] );
    end
...
%% Plot 3D cylindrical
% #########################################################################
function plot_3D_Cylindrical( t,p,r,PT ,thetai,phii )
...
% Convert spherical coordinates to Cartesian coordinates for Incident Wave
if ~isnan(thetai) 
    ui = thetai.*cosd(phii);     
    vi = thetai.*sind(phii);
    % Plotting white dot for the incident wave
    plot3(ui,vi,0,'o','Color','w','MarkerFaceColor','w','MarkerSize',6)
end
...
%% Plot 3D spherical
% #########################################################################
function plot_3D_Spherical( t,p,r,PT,thetai,phii )
...
% Creating a line from where the incident array starts
if ~isnan(thetai) 
% Convert spherical coordinates to Cartesian coordinates for Incident Wave
xi = r * sind(thetai) * cosd(phii);
yi = r * sind(thetai) * sind(phii);
zi = r * cosd(thetai);
% Plotting the point representing the incident direction
plot3(xi, yi, zi, '-o', 'MarkerSize', 7, 'MarkerFaceColor', 'red', 'MarkerEdgeColor', 'black');
end
